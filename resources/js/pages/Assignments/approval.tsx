...
export default function show({ assignments, settingApproval, approvals }: PrintAssignments) {
    ...
    const handleSubmit2 = (e: React.FormEvent) => {
        ...
        if (!selectedUserId) {
            toast.error('User approver belum dipilih.');
            return;
          }
        const signature = sigPad.current?.toDataURL('image/png');
        // setData('signature', signature || '');
    ...
    return (
        <>
            ...
            <li>Tight VNC</li> */}
                ...
                {/* Tanda Tangan */}
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 text-center mt-10">
                    {settingApproval
                            ...
                            return (
                                <div key={setting.id} className="border p-3 rounded mb-2">
                                    ...
                                    {approval ? (
                                        ...
                                        <Button onClick={() => {
                                            setSelectedUserId(setting.user_id);
                    ...
                    {settingApproval
                        ...
                        .map((setting) => {
                         // cari approval untuk user_id approver ini
                            const approval = approvals?.find(
                                (a) => Number(setting.user_id) === Number(a.user_id)
                            );
  ...
  if (!open) {
    // reset signature saat modal ditutup
    sigPad.current?.clear();
  }
}}>
                <AlertDialogContent className="w-[95vw] sm:max-w-[560px]">
                    <AlertDialogHeader>
                    ...
                    >
                        <SignatureCanvas
                            ...
                            canvasProps={{
                                width: 400,
                                height: 200,
                                className: 'w-full max-w-[400px] h-auto border border-gray-300 rounded-md block mx-auto'
                            }}
                        />
                        ...
                        <div className="flex gap-3">
                            ...
                            <Button variant="destructive" onClick={() => sigPad.current?.clear()}>Clear</Button>
            ...
            <AlertDialog open={isDialogOpen2} onOpenChange={(open) => {
  ...
  <AlertDialogContent className="w-[95vw] sm:max-w-[560px]">
    ...
    <form onSubmit={handleSubmit2} encType="multipart/form-data" className="flex flex-col items-center gap-4">
      <SignatureCanvas
        penColor="black"
        ref={sigPad}
        canvasProps={{ width: 400, height: 200, className: 'w-full max-w-[400px] h-auto border border-gray-300 rounded-md block mx-auto' }}
      />

      <div className="flex gap-3">
        <Button type="button" variant="destructive" onClick={() => sigPad.current?.clear()}>
          Clear
        </Button>

        <Button type="submit">simpan</Button>

        <AlertDialogCancel onClick={() => setIsDialogOpen2(false)}>
          Cancel
        </AlertDialogCancel>
      </div>
    </form>
